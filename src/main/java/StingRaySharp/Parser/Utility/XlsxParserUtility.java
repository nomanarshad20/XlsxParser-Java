...
public class XlsxParserUtility {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String inFile2 = "C:\\Users\\noman\\Documents\\042621364LA start.xlsx";

			String outFile = "C:\\Users\\noman\\Desktop\\StartLienSummaryChart" + new Date().getMinutes() + "_"
	...
	public static String readXlsx(String inFile, String outFile) throws IOException {
		...
		try {
			...
			while (myListIterator.hasNext()) {
				...
				try {
					debtrNames = innerDataList.get(0);
				} catch (Exception e) {
				}

				try {
					jurisdiction = innerDataList.get(1);
				} catch (Exception e) {
				}

				try {
					service = innerDataList.get(2);
				} catch (Exception e) {
				}

				try {
					dateThru = innerDataList.get(3);
				} catch (Exception e) {
				}
				...
				/*
				 * System.out.println( debtrNames ); System.out.println(
				 * jurisdiction ); System.out.println( dateThru );
	...
	private static void groupByState(List<Map<String, String>> preparedDebtorList,
		...
		if (!preparedDebtorList.isEmpty()) {
			...
			while (rawDataListIterator.hasNext()) {
				...
				if (innerMap.get("State").equalsIgnoreCase(state)
						&& innerMap.get("Debtor Name").equalsIgnoreCase(dbtrName)) {
			...
			if (match) {
				...
				finalRowDataMap.put(dbtrName + " " + "(" + StateUsAbriviationEnums.getKeyByValue(state) + ")",
						prepListForRowDataMap);
			}
		}
	...
	private static void getNextDebtorRecord(List<Map<String, String>> preparedDebtorList,
